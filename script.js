...
const firebaseConfig = {
	apiKey: "AIzaSyAxNMcP74QOPwEmAOC3V2wnQejlyWYwkvo",
	authDomain: "mostafa-zienab-wedding.firebaseapp.com",
	projectId: "mostafa-zienab-wedding",
	storageBucket: "mostafa-zienab-wedding.firebasestorage.app",
	messagingSenderId: "144448957037",
	appId: "1:144448957037:web:0194ee80cd2f5ed85ced64",
};
...
document.addEventListener("DOMContentLoaded", () => {
	...
	// 6. Wishes Guestbook Logic (Firebase Firestore)
	let allWishes = [];

	function formatWishDate(createdAt) {
		const date = createdAt?.toDate ? createdAt.toDate() : createdAt instanceof Date ? createdAt : new Date();
	...
	function renderWishes() {
		...
		if (allWishes.length === 0) {
			const emptyText = isRtl ? "كن أول من يكتب تهنئة للعروسين في دفتر التهاني ❤️✨" : "Be the first to leave your congratulations for the couple! ❤️✨";
			wishesFeed.innerHTML = `
      ...
      `;
        ...
        <div class="wish-header">
          ...
          <span class="wish-date">${escapeHTML(formatWishDate(item.createdAt))}</span>
        </div>
	...
	function subscribeToWishes() {
		...
		onSnapshot(
			wishesQuery,
			(snapshot) => {
				allWishes = snapshot.docs.map((docSnap) => {
			...
			(err) => {
				console.error("Failed to load guestbook wishes:", err);
			},
		);
	}

	async function addWishToList(name, text) {
		...
		try {
			...
			alert(isRtl ? "تعذر حفظ التهنئة. حاول مرة أخرى." : "Could not save your wish. Please try again.");
		}
	}
	...
	wishesForm.addEventListener("submit", async (e) => {
		...
		const submitBtn = wishesForm.querySelector(".btn-submit");

		if (!wName.trim() || !wText.trim()) return;

		if (submitBtn) submitBtn.disabled = true;
		await addWishToList(wName, wText);
		wishesForm.reset();
